---
title: "Note_AboutData_H24071207拾可信"
author: "H24071207拾可信"
date: "2020/5/5"
...
# 1.Remove NA
## when NA occur
1.is.na(X)：use in vector  
2.complete.cases(X)：use in vector, matrix and data frame  
3.na.omit(X)
```{r}
#Ex
airquality[1:8,]
...
# 2.Date process
## use cbind and rbind to combine data
## use cat to classify data or give data a name tag
```{r}
#將資料中的變數"wind"分組
...
# 4.Regression
## Display the relation between to variance
## Need variance relation chart to confirm to relation before data anaylizing. 
```{r}
