---
title: "Note_Graphic_H24071207拾可信"
author: "H24071207拾可信"
date: "2020/5/6"
output: html_document
---
# 1.Statistical Figures
### (a).Graphics for descriptive statistics
```{r}
...
cor(x, y)
```
### (b).Scatter plots for multiple variables
```{r}
#Use pairs(. . . ).
...
### (c).Boxplot for multiple categories
```{r}
#use matrix
...
# 2.Creat a plot region with specifical items

### (a).General plot function: plot(. . . )
```{}
plot(x, y,
...
Size of points: cex = 
Size of the title: cex.main = 
Size of the labels of ticks of an axis: cex.axis = 
Size of the labels of the name of an axis: cex.lab = 
```
...
curve(5*x+2, 0, 10)
```
# 3.Add some objects to the existing plot
### (a).Add points or lines to an existing plot
```{r}
#points(. . . ): add points
#lines(. . . ): add lines
...
### (b).Add text to an existing plot
```{r}
#Arbitrary locations:
#text(x, y, labels = . . . )
...
### (c).Re-arrange the region of a figure
```{r}
...
#mai, mar = c(3, 3, 1, 1): The four margin sizes, in inches (mai), or intext line units (mar).
